# Grounded in ser9's ~/hivemind-bridges/hackchat/run.sh:
#   hivemind-hackchat-bridge --channel ... --username ... --access-key ... \
...
ENV XDG_CONFIG_HOME=/config \
    ...
    HIVEMIND_PORT=5678 \
    HACKCHAT_CHANNEL=hivemind-selfhost \
    HACKCHAT_USERNAME=HiveMindBot \
    CREDS_DIR=/creds

ENTRYPOINT ["/usr/local/bin/wait-for-creds.sh", "hackchat", "--"]
CMD ["sh", "-c", "exec hivemind-hackchat-bridge \
  --channel \"$HACKCHAT_CHANNEL\" --username \"$HACKCHAT_USERNAME\" \
